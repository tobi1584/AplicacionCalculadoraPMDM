...
class Masa : AppCompatActivity() {
    ...
    private val conversionMap = mapOf(
        ...
        "Microgramo μg" to 0.000001,
        "Quintal q" to 100_000.0,
        "Libra lb" to 453.592,
        "Onza oz" to 28.3495,
        "Quilate ct" to 0.2,
        "Grano gr" to 0.06479891,
        "Tonelada larga l.t" to 1_016_046.9088,
        "Tonelada corta sh.t" to 907_184.74,
        "Quintal ingles cwt" to 45_359.237,
        "Quintal norteamericano cwt" to 50_000.0
    )
    ...
    private fun performConversion(finalize: Boolean) {
        ...
        val formattedValue = if (convertedValue % 1.0 == 0.0) {
            NumberFormat.getInstance(locale).format(convertedValue.toLong())
        } else {
        ...
        outputEditText.setText(formatForDisplay(formattedValue))
    ...
    private fun formatForDisplay(value: String): String {
        ...
        return if (number != null) {
            ...
            val groupingSeparator = nf.decimalFormatSymbols.groupingSeparator.toString()
            val digitCount = value.replace(groupingSeparator, "").length
            if (digitCount > 9) {
                try {
